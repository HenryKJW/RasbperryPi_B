...
volatile uint32_t *bcm2835_dma = MAP_FAILED;
...
static int i2c_byte_wait_us = 0;// 0
...
void bcm2835_pwm_set_channel_out(uint8_t channel, uint8_t outmode, uint8_t datafrom)
{
  uint32_t control = bcm2835_peri_read(bcm2835_pwm + BCM2835_PWM_CONTROL);
...
void bcm2835_pwm_reset_pwm_enable(uint8_t channel)
{
  uint32_t control = bcm2835_peri_read(bcm2835_pwm + BCM2835_PWM_CONTROL);
...
void bcm2835_pwm_set_fifo_ctl(uint8_t channel, uint8_t repeatlastdata)
{
  uint32_t control = bcm2835_peri_read(bcm2835_pwm + BCM2835_PWM_CONTROL);
...
void bcm2835_pwm_reset_fifo(uint8_t cleanfifo)
{
  uint32_t control = bcm2835_peri_read(bcm2835_pwm + BCM2835_PWM_CONTROL);
  
  if (cleanfifo)
  {
	control |= BCM2835_PWM_CLEAR_FIFO;
	// If you use the barrier here, wierd things happen, and the commands dont work
	bcm2835_peri_write_nb(bcm2835_pwm + BCM2835_PWM_CONTROL, control);
	bcm2835_delayMicroseconds(100);
  }
...
void bcm2835_pwm_set_fifo_data(uint32_t fifodata)
{
  bcm2835_peri_write_nb(bcm2835_pwm + BCM2835_PWM_FIF1, fifodata);  
}

void bcm2835_pwm_set_DMA(uint8_t enable)
{
  uint32_t control = bcm2835_peri_read(bcm2835_pwm + BCM2835_PWM_DMAC);

  if(enable)
	control |= BCM2835_PWM_DMA_ENABLE;
  else
	control &= ~BCM2835_PWM_DMA_ENABLE;
  bcm2835_peri_write_nb(bcm2835_pwm + BCM2835_PWM_DMAC,  control | BCM2835_PWM_DMA_PANIC_DREQ);
}
...
///MemSize = PAGE_SIZE * n
void bcm2835_DMA_SetupDMA(void** DMAPage){
      uint32_t control;

//	atexit(bcm2835_DMA_unSetupDMA);
	...
	//activate dma
 	BCM2835_DMA_REGs* DMA0 = (BCM2835_DMA_REGs*) bcm2835_dma;

	DMA0->CS =BCM2835_DMA_CS_Reset;
	DMA0->CONBLK_AD=0; 
 	DMA0->TI=0;
	DMA0->CONBLK_AD = (unsigned int) *DMAPage;
	
	control = bcm2835_peri_read(bcm2835_dma + BCM2835_DMA_CONBLK_AD);
	printf("bcm2835_DMA_SetupDMA2: %x ----------------------\n", control);

	DMA0->CS =BCM2835_DMA_CS_Enable |(0xFF <<16);// enable bit = 0, clear end flag = 1, prio=19-16, pan_prio=23~20
}

void bcm2835_DMA_unSetupDMA(void){
	BCM2835_DMA_REGs* DMA1 = (BCM2835_DMA_REGs*) bcm2835_dma;
	DMA1->CS =BCM2835_DMA_CS_Reset;
	printf("exiting\n");
}

//CM_GP0CTL ---------------------------------------------------------------
void bcm2835_cm_set_clock(uint8_t mashcontrol, uint8_t clocksource, uint32_t divisor_integer, uint32_t divisor_farctional)
{
  // From Gerts code
  divisor_integer &= 0xfff;
  divisor_farctional &= 0xfff;
  // Stop PWM clock
  bcm2835_peri_write(bcm2835_clk + BCM2835_CM_GP0CTL, BCM2835_PWM_PASSWRD | (mashcontrol <<9) |clocksource); // Source=osc and enable  
  CLRBIT(bcm2835_clk + BCM2835_CM_GP0CTL, BCM2835_CM_GPxCTL_ENAB);
  bcm2835_delay(110); // Prevents clock going slow
  // Wait for the clock to be not busy
  while ((bcm2835_peri_read(bcm2835_clk + BCM2835_CM_GP0CTL) & BCM2835_CM_GPxCTL_BUSY) != 0)
    bcm2835_delay(1); 
  // set the clock divider and enable PWM clock
  bcm2835_peri_write(bcm2835_clk + BCM2835_CM_GP0DIV, BCM2835_PWM_PASSWRD | (divisor_integer << 12));
  bcm2835_peri_write(bcm2835_clk + BCM2835_CM_GP0CTL, BCM2835_PWM_PASSWRD | (mashcontrol <<9) |clocksource |BCM2835_CM_GPxCTL_ENAB); // Source=osc and enable
}

//ALL Clock Setting (GPx_CLK-0x7E101070¡BPWM_CLK-0x7E101040¡BPCM_CLK-0x7E101098¡BPWM_CLK=0x7E1010A0)
void bcm2835_all_set_clock(uint8_t clocktype, uint8_t mashcontrol, uint8_t clocksource, uint32_t divisor_integer, uint32_t divisor_farctional)
{
  // From Gerts code
  volatile uint32_t *  MMIOAddress;
  ...
  switch(clocktype)
  {
  	case BCM2835_CM_PWMCLK:
		MMIOAddress = bcm2835_clk+BCM2835_PWMCLK_CNTL;
		break;
	
  	case BCM2835_CM_GP0CLK:
		MMIOAddress = bcm2835_clk+BCM2835_CM_GP0CTL;
		break;
	
  	case BCM2835_CM_GP1CLK:
		MMIOAddress = bcm2835_clk+BCM2835_CM_GP1CTL;
		break;
	
  	case BCM2835_CM_GP2CLK:
		MMIOAddress = bcm2835_clk+BCM2835_CM_GP2CTL;
		break;
  ...
  // Stop PWM clock
  bcm2835_peri_write(MMIOAddress, BCM2835_PWM_PASSWRD | (mashcontrol <<9) |clocksource); // Source=osc and enable  
  CLRBIT(MMIOAddress, BCM2835_CM_GPxCTL_ENAB);
  bcm2835_delay(110); // Prevents clock going slow
  // Wait for the clock to be not busy
  while ((bcm2835_peri_read(MMIOAddress) & BCM2835_CM_GPxCTL_BUSY) != 0)
    bcm2835_delay(1); 
  // set the clock divider and enable PWM clock
  bcm2835_peri_write(MMIOAddress+1, BCM2835_PWM_PASSWRD | (divisor_integer << 12));
  bcm2835_peri_write(MMIOAddress, BCM2835_PWM_PASSWRD | (mashcontrol <<9) |clocksource |BCM2835_CM_GPxCTL_ENAB); // Source=osc and enable
}
...
int bcm2835_init(void)
{
	...
	bcm2835_dma = (uint32_t*)BCM2835_DMA_BASE; 
    ...
    //DMA
    bcm2835_dma = mapmem("dma", BCM2835_BLOCK_SIZE, memfd, BCM2835_DMA_BASE);
    if (bcm2835_dma == MAP_FAILED) goto exit;
...
int bcm2835_close(void)
{
    ...
    unmapmem((void**) &bcm2835_dma, BCM2835_BLOCK_SIZE);
